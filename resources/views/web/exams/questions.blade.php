      ...
      <div class="row">
          <div class="col-md-10 col-md-offset-1 text-center">
              <ul class="hero-area-tree">
                  ...
                  <li><a href="{{url("cats/show/{$exam->skill->cat->id}")}}">{{$exam->skill->cat->name()}}</a></li>
                  <li><a href="{{url("skills/show/{$exam->skill->id}")}}">{{$exam->skill->name()}}</a></li>
                  <li>{{$exam->name()}}</li>
              </ul>
              <h1 class="white-text">{{$exam->name()}}</h1>
              <ul class="blog-post-meta">
                  <li>{{carbon\carbon::parse($exam->created_at)->format('y M, Y')}}</li>
...
<form  id="exam-submit-form" action="{{url("exams/submit/{$exam->id}")}}" method="post">
  @csrf
                ...
                <div class="blog-post mb-5">
                    <p>
                      @foreach ($exam->questions as $key => $item)

                      <div class="panel panel-default">
                          <div class="panel-heading">
                            <h3 class="panel-title">{{ ($key+1) . " - ".$item->title}}</h3>
                          </div>
                          <div class="panel-body">
                              <div class="radio">
                                  <label>
                                    <input type="radio" name="answers[{{$item->id}}]"   value="1" form="exam-submit-form">
                                   {{$item->option_1}}
                                  </label>
                              </div>
                              <div class="radio">
                                  <label>
                                    <input type="radio" name="answers[{{$item->id}}]"    value="2" form="exam-submit-form">
                                    {{$item->option_2}}
                                  </label>
                              </div>
                              <div class="radio">
                                  <label>
                                    <input type="radio" name="answers[{{$item->id}}]"   value="3" form="exam-submit-form">
                                      {{$item->option_3}}
                                  </label>
                              </div>
                              <div class="radio">
                                  <label>
                                    <input type="radio" name="answers[{{$item->id}}]"   value="4" form="exam-submit-form">
                                      {{$item->option_4}}
                                  </label>
                              </div>
                          </div>
            ...
            <div id="aside" class="col-md-3">
              ...
              <div class="duration_countdown" data-timer="{{$exam->duration_mins * 60}}"></div>
              {{-- <div class="duration_countdown" data-timer="5"></div> --}}
...
//$(".duration_countdown").TimeCircles({start: false});
$(".duration_countdown").TimeCircles({
  time : {
    Days : {
      show :false,
    }
